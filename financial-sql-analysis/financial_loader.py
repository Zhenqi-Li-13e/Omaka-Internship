...
def load_trade_calendar():
    ...
    conn.close()
    return cursor.execute('SELECT COUNT(*) FROM trade_calendar').fetchone()[0]
...
def load_etf_data():
    ...
    try:
        ...
        # 筛选ETF（基金简称中包含"ETF"的）
        etf_df = fund_df[fund_df['基金简称'].str.contains('ETF', na=False)].copy()
        ...
        # 深交所: 159xxx, 160xxx, 161xxx
        def add_exchange_suffix(code):
            code = str(code).strip()
            if code.startswith(('510', '511', '512', '513', '515', '516', '517', '518', '588')):
                return code + '.SH'
            elif code.startswith(('159', '160', '161', '162', '163')):
                return code + '.SZ'
            return code

        etf_df['ts_code'] = etf_df['基金代码'].apply(add_exchange_suffix)
